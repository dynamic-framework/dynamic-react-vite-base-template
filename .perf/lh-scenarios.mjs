/**
 * Variante de lh.mjs para la validacion de preserveModules: permite elegir
 * escenarios y etiqueta de salida.
 * Uso: node .perf/lh-scenarios.mjs <etiqueta> <escenarios: 1,3> <corridas>
 */
...
for (const n of SCENARIOS) {
  for (let run = 1; run <= RUNS; run++) {
    ...
    rows.push(row);
    console.log(
      `[${LABEL}] n=${n} run=${run} score=${row.perfScore} fcp=${row.fcpMs.toFixed(0)} `
      + `lcp=${row.lcpMs.toFixed(0)} tbt=${row.tbtMs.toFixed(0)} si=${row.siMs.toFixed(0)} `
      + `bootup=${(row.bootupMs ?? 0).toFixed(0)}`,
    );
    await chrome.kill();
  }
}
writeFileSync(new URL(`results-lh-${LABEL}.json`, import.meta.url), JSON.stringify(rows, null, 2));
console.log('escrito results-lh-' + LABEL + '.json');
