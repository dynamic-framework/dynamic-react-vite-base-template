 ...
 *
 * Construye .perf/widget-entry-motion.tsx como un bundle único `widget.js` con los
...
export default defineConfig({
  plugins: [
    ...
    lucideSubset({ include: [], strict: false }),
    motionStub(),
  ],
  resolve: {
    alias: {
  ...
  build: {
    outDir: 'dist-cf-behav4',
    cssCodeSplit: false,
    chunkSizeWarningLimit: 2000,
    minify: 'esbuild',
    assetsDir: '',
    rollupOptions: {
      input: path.resolve(__dirname, '.perf/widget-entry-motion.tsx'),
      output: {
